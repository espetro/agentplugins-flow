...
## `trace` — replay prior tool results

Lightweight session-history lookup. No model call, no child process. Pass the exact `tool_call_id` values from prior tool calls and get the verbatim results back in the order you specify, followed by your remark.

### Tool parameters

| Parameter | Type | Description |
|-----------|------|-------------|
| `ids` | `string[]` | Ordered list of prior `tool_call_id` values to replay |
| `remark` | `string` | Synthesis or reasoning appended after the results (comes last so evidence is seen first) |

### Behavior

- **Order preserved** — results are returned in the exact order of the `ids` array.
- **Missing IDs silently skipped** — if an ID is not found in the session history, it produces no output (no error noise).
- **Strict output order** — all tool results first, then the remark.
- **Duplicate IDs allowed** — requesting the same ID twice yields the result twice.
...
```json
{
  ...
  "remark": "Both results confirm the fix is safe."
}
```
...
```
<file content from call-123>

<bash output from call-456>

Both results confirm the fix is safe.
```
