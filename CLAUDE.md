...
## Bash-style tool surface (Phase 3–5)

As of v2.3.0, the four primary tools (`batch_read`, `batch`, `trace`, `flow`) share a single bash-style CLI surface: one `cmd: string` field per tool, subcommand + flag dispatch, and `;`/`&&` chaining. See [docs/TOOLS.md](docs/TOOLS.md) for per-tool grammar.

| Tool | Schema | One-line example |
|------|--------|------------------|
| `batch_read` | `{ cmd: string, cwd?: string }` | `cmd: "read src/index.ts:10-50"` |
| `batch` | `{ cmd: string, cwd?: string }` | `cmd: "read foo.txt; rg -q pattern src/; web fetch -u https://..."` |
| `trace` | `{ cmd: string, cwd?: string }` | `cmd: "trace --intent 'verify auth' -- batch read src/auth.ts"` |
| `flow` | `{ cmd: string, cwd?: string }` | `cmd: "flow --type scout --intent 'Map' --aim 'Map' --concern 'x' --complexity simple"` |

The new surface replaced a JSON mega-schema (`{ tool, ops: [{ o, p, c, e, ... }] }`) with one terse string. Implementation lives in `src/cli/` (`parse.ts`, `chain.ts`, `tokenize.ts`, `flags/`, `exec/`, `register.ts`). Pre-flight dispatch for `trace`/`flow` is now a single `cmd: "..."` passed after `--`, parsed by the same `chain`/`tokenize` helpers.
