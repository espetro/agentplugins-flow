...
describe("parseFlowCliArgs", () => {
  ...
  it("extracts --flow-complexity", () => {
    const result = parseFlowCliArgs(["node", "script", "--flow-complexity", "complex"]);
    expect(result.flowComplexity).toBe("complex");
  });

  it("extracts --flow-complexity with equals syntax", () => {
    const result = parseFlowCliArgs(["node", "script", "--flow-complexity=simple"]);
    expect(result.flowComplexity).toBe("simple");
  });

  it("rejects invalid --flow-complexity values", () => {
    const result = parseFlowCliArgs(["node", "script", "--flow-complexity", "900"]);
    expect(result.flowComplexity).toBeUndefined();
  });
